		...
		for (var i=0;i<group.length;i++) {
			...
			for (var k=0;k<group_array.length;k++){
				...
				if ('size' in options){
					...
					console.log(this.dataset[options['size']])
					...
					console.log(minsize,maxsize,pointsize)
				...
				if (options['points']==true) {
					snapobj.circle(x_data_value,y_data_value,pointsize).attr({fill:this.qualitative_color[i],stroke:this.qualitative_color[i],'stroke-width':this.point_strokewidth,'data_type':'point','data_label':group_array[k][4],'group':group[i],'class':'dataelement'})
				}
