...
function draw_axes(playobj,xvar,yvar,shiftx,shifty) {
	...
	if(total_xoffset>.15*playobj.width){total_xoffset=.15*playobj.width}
	...
	for(var i=0;i<xvar.length+shiftx;i++){
		...
		var temp_tick=snapobj.line(xstart_xcoord+x_step*i,playobj.y+playobj.height-coords.height-playobj.xtick_to_xaxis-playobj.footer_bottompad-playobj.logo_height-playobj.footer_toppad-playobj.bottom_margin-xlab_height-playobj.xtick_to_xlabel,xstart_xcoord+x_step*i,playobj.y+playobj.height-coords.height-(playobj.xtick_to_xaxis/2)-playobj.footer_bottompad-playobj.logo_height-playobj.footer_toppad-playobj.bottom_margin-xlab_height-playobj.xtick_to_xlabel).attr({stroke:playobj.xtick_fill,'stroke-width':playobj.xtick_thickness})
	...
	for(var i=0;i<yvar.length+shifty;i++){
		...
		var temp_line=snapobj.line(playobj.x+total_xoffset,playobj.y+playobj.height-total_yoffset-y_step*i,playobj.x+playobj.width-playobj.right_margin,playobj.y+playobj.height-total_yoffset-y_step*i).attr({stroke:playobj.ygrid_fill})
		coords=temp_line.getBBox()
		temp_line.attr({y1:Math.floor(coords.y)+.5,y2:Math.floor(coords.y)+.5})

		var temp_minorline=

		var temp_tick=snapobj.line
	}
}
