	...
	Playfair.prototype.linechart = function(options) {
		...
		for (var i=0;i<group.length;i++) {
			...
			for (var k=0;k<group_array.length;k++){
				...
				snapobj.circle(x_data_value,y_data_value,5).attr({fill:'white'})
				snapobj.circle(x_data_value,y_data_value,3.5).attr({fill:graph_obj.sequential_color[2]})
			}
		}
		...
		if (typeof options['abline']!==undefined){
			...
			console.log(x1_data_value,y1_data_value,x2_data_value,y2_data_value)
			var trend=snapobj.line(x1_data_value,y1_data_value,x2_data_value,y2_data_value).attr({stroke:'#c64027','stroke-width':2})
		}
	}
