...
window.playfair = (function () {
	...
	Playfair.prototype.linechart = function(options) {
		...
		data_array=[]
		for(var i=0;i<this.dataset[this.yvar].length;i++){
			data_array.push([this.dataset[this.yvar][i],this.dataset[this.xvar][i],grouptemp[i]])
		}
		...
		for (var i=0;i<group.length;i++) {
			...
			for (var k=0;k<group_array.length;k++){
				...
				snapobj.circle(x_data_value,y_data_value,this.point_size).attr({fill:this.point_stroke})
				snapobj.circle(x_data_value,y_data_value,this.point_size-this.point_strokewidth).attr({fill:this.point_fill})
			}
		}

		// draw abline if necessary
		console.log(options['abline'])
		if (typeof options['abline']!==undefined){
			...
			console.log(coords)
			trendtext.attr({y:coords.y-this.trend_linetotext})
			inclination=-(Math.atan(parseFloat(slope))*360)/(2*Math.PI)
			trendtext.transform('r'+inclination+' '+coords.cx+' '+coords.cy)
			console.log('r'+inclination)
		}
	}
	...
	Playfair.prototype.style = function(parameters) {
		...
		this.trend_linetotext= parameters['trend_linetotext'] || '6'
		this.point_fill= parameters['point_fill'] || '#67c2a5'
		this.point_stroke= parameters['point_stroke'] || 'white'
		this.point_size=4.5
		this.point_strokewidth=1.5
