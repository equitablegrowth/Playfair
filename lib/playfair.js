...
window.playfair = (function () {
	...
	Playfair.prototype.style = function() {
		...
		this.xgrid_minordasharray=[3,3] //[1,0]
...
function draw_axes(playobj,xvar,yvar,shiftx,shifty) {
	...
	for(var i=0;i<xvar.length+shiftx;i++){
		...
		var temp_line=snapobj.line(xstart_xcoord+x_step*i,y_start,xstart_xcoord+x_step*i,y_end).attr({stroke:playobj.xgrid_fill,'stroke-width':playobj.xgrid_thickness,'stroke-dasharray':playobj.xgrid_dasharray})
		var temp_minorline=snapobj.line(xstart_xcoord+(x_step/2)+x_step*i,y_start,xstart_xcoord+(x_step/2)+x_step*i,y_end).attr({stroke:playobj.xgrid_minorfill,'stroke-width':playobj.xgrid_minorthickness,'stroke-dasharray':playobj.xgrid_minordasharray})
		...
		coords2=temp_minorline.getBBox()
		temp_tick.attr({x1:Math.floor(coords.x)+.5,x2:Math.floor(coords.x)+.5})
		temp_line.attr({x1:Math.floor(coords.x)+.5,x2:Math.floor(coords.x)+.5})
		temp_minorline.attr({x1:Math.floor(coords2.x)+.5,x2:Math.floor(coords2.x)+.5})
