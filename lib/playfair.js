...
window.playfair = (function () {
	...
	Playfair.prototype.linechart = function(options) {
		...
		// connect variable - how should points be ordered for joining: [varname]
		...
		for (var i=0;i<group.length;i++) {
			...
			for (var k=0;k<group_array.length;k++){
				...
				if (options['points']==true) {
					snapobj.circle(x_data_value,y_data_value,this.point_size).attr({fill:this.point_fill,stroke:this.qualitative_color[i],'stroke-width':this.point_strokewidth,'data_type':'point','data_label':'a','group':group[i],'class':'dataelement'})
				}
			}
		}
