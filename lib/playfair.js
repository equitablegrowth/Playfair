...
window.playfair = (function () {
	...
	Playfair.prototype.style = function() {
		...
		this.xgrid_thickness=1 //0
		this.xgrid_zerothickness=0
		this.xgrid_minorthickness=1 //0
		this.xgrid_dasharray=[1,0]
		this.xgrid_zerodasharray=[1,0]
		this.xgrid_minordasharray=[2,2] //[1,0]
		...
		this.xtick_to_xlabel=6
		this.xtick_to_xaxis=6
...
function draw_axes(playobj,xvar,yvar,shiftx,shifty) {
	...
	total_yoffset=playobj.y+playobj.width
	if(shiftx==1){shiftx=1}
	else{shiftx=0}

	for(var i=0;i<xvar.length+shiftx;i++){
		lines=multitext(String(xvar[i]),{ident:'xaxis','font-size':playobj.xtick_textsize,'font-weight':playobj.xlabel_textweight,'font-face':playobj.xlabel_textface,'dominant-baseline':'text-before-edge','text-anchor':'middle'},playobj.xtick_maxsize*playobj.width)
		for(var j=0;j<lines.length;j++){
			var temp=snapobj.text(0,playobj.y+playobj.height-playobj.bottom_margin-playobj.logo_height-playobj.footer_toppad-playobj.footer_bottompad-xlab_height-xtick_to_xlabel-(lines.length-i)*playobj.xtick_textsize,lines[i]).attr({ident:'xaxis','font-size':playobj.xtick_textsize,'font-weight':playobj.xlabel_textweight,'font-face':playobj.xlabel_textface,'dominant-baseline':'text-before-edge','text-anchor':'middle'})
		}
		coords=temp.getBBox()
	...
	total_yoffset=playobj.y+playobj.height-total_yoffset+playobj.xtick_to_xaxis
	...
	for(var i=0;i<xvar.length+shiftx;i++){
		...
		// x-axis labels
		lines=multitext(String(xvar[i]),{ident:'xaxis','font-size':playobj.xtick_textsize,'font-weight':playobj.xlabel_textweight,'font-face':playobj.xlabel_textface,'dominant-baseline':'text-before-edge','text-anchor':'middle'},playobj.xtick_maxsize*playobj.width)
		var temp=snapobj.text(xstart_xcoord+(x_step/2)*shiftx+x_step*i,playobj.y+playobj.height-playobj.footer_bottompad-playobj.logo_height-playobj.footer_toppad-playobj.bottom_margin-xlab_height-playobj.xtick_to_xlabel,String(xvar[i])).attr({ident:'xaxis','font-size':playobj.xtick_textsize,'font-weight':playobj.xlabel_textweight,'font-face':playobj.xlabel_textface,'dominant-baseline':'text-before-edge','text-anchor':'middle'})
		coords=temp.getBBox()
		...
		var temp_line=snapobj.line(xstart_xcoord+x_step*i,y_start,xstart_xcoord+x_step*i,y_end).attr({stroke:playobj.xgrid_fill,'stroke-width':playobj.xgrid_thickness,'stroke-array':playobj.xgrid_dasharray})
		var temp_minorline=snapobj.line(xstart_xcoord+(x_step/2)+x_step*i,y_start,xstart_xcoord+(x_step/2)+x_step*i,y_end).attr({stroke:playobj.xgrid_minorfill,'stroke-width':playobj.xgrid_minorthickness,'stroke-array':playobj.xgrid_minordasharray})
		var temp_tick=snapobj.line(xstart_xcoord+x_step*i,y_start,xstart_xcoord+x_step*i,y_start+playobj.xtick_length).attr({stroke:playobj.xtick_fill,'stroke-width':playobj.xtick_thickness})
		
		// place all lines at int+.5 for rendering
		// coords=temp_tick.getBBox()
		// temp_tick.attr({x1:Math.floor(coords.y)+.5,x2:Math.floor(coords.x)+.5})
		// temp_line.attr({x1:Math.floor(coords.y)+.5,x2:Math.floor(coords.x)+.5})
		// temp_minorline.attr({x1:Math.floor(coords.y)+.5,x2:Math.floor(coords.x)+.5})
	}

	// important parameters - the start and end of the y axis, range, and y step size
	ystart_ycoord=playobj.y+playobj.height-total_yoffset
	...
	for(var i=0;i<yvar.length+shifty;i++){
		// y-axis labels
		var temp=snapobj.text(playobj.x+total_xoffset-playobj.ytick_to_yaxis,ystart_ycoord-(y_step/2)*shifty-y_step*i,String(yvar[i])).attr({ident:'yaxis','font-size':playobj.ytick_textsize,'font-weight':playobj.ylabel_textweight,'font-face':playobj.ylabel_textface,'dominant-baseline':'text-before-edge','text-anchor':'end'})
		coords=temp.getBBox()
		temp.attr({y:coords.y-(coords.height/2)})

		// y-axis ticks, grid lines, and minor grid lines
		var temp_line=snapobj.line(playobj.x+total_xoffset,playobj.y+playobj.height-total_yoffset-y_step*i,playobj.x+playobj.width-playobj.right_margin,playobj.y+playobj.height-total_yoffset-y_step*i).attr({stroke:playobj.ygrid_fill,'stroke-width':playobj.ygrid_thickness,'stroke-dasharray':playobj.ygrid_dasharray})
		var temp_minorline=snapobj.line(playobj.x+total_xoffset,playobj.y+playobj.height-total_yoffset-(y_step/2)-y_step*i,playobj.x+playobj.width-playobj.right_margin,playobj.y+playobj.height-total_yoffset-y_step*i).attr({stroke:playobj.ygrid_minorfill,'stroke-width':playobj.ygrid_minorthickness,'stroke-dasharray':playobj.ygrid_minordasharray})
		var temp_tick=snapobj.line(playobj.x+total_xoffset,playobj.y+playobj.height-total_yoffset-(y_step/2)-y_step*i,playobj.x+playobj.width-playobj.right_margin,playobj.y+playobj.height-total_yoffset-y_step*i).attr({stroke:playobj.ytick_fill,'stroke-width':playobj.ytick_thickness})

		// place all lines at int+.5 for rendering
		coords=temp_line.getBBox()
		temp_tick.attr({y1:Math.floor(coords.y)+.5,y2:Math.floor(coords.y)+.5})
		temp_line.attr({y1:Math.floor(coords.y)+.5,y2:Math.floor(coords.y)+.5})
		temp_minorline.attr({y1:Math.floor(coords.y)+.5,y2:Math.floor(coords.y)+.5})
