		...
		if(orientation=='vertical'){
			...
			for (var i=0;i<xaxis.length;i++){
				...
				if(typeof this.group!=='undefined'){
					...
					temp_x=[]
					for (var k=0;k<this.dataset[this.xvar].length;k++){
						if (this.dataset[this.xvar][k]==xaxis.length[i]){
							temp_x.push(k)
						}
					}

					acc_k=0
					for (var j=0;j<temp_x.length;j++){
						for (var k=0;k<group.length;k++){
							if (this.dataset[this.group][temp_x[j]]==group[k]){
								data_value=(this.dataset[this.yvar][j])/(yaxis[yaxis.length-1]-yaxis[0])*(axes[2]-axes[3])
								var bar_rect=snapobj.rect(axes[0]+margin*(i+1)+margin*i+bar_width*i+bar_width*acc_k,zero-data_value,bar_width,data_value).attr({'data_type':'bar','data_label':this.dataset[this.yvar][j],'group':group[k],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[k]})
								acc_k=acc_k+1
							}
						} 
					}
