...
window.playfair = (function () {
	...
	Playfair.prototype.header = function(hed,dek) {
		...
		console.log('hedsizemin: ',this.hedsizemin,hedfontsize)
...
function draw_axes(playobj,xvar,yvar,shiftx,shifty) {
	...
	if (typeof(playobj.ylabel)!=undefined){
		...
		ylab.attr({y:ylab_coords.y-(ylab_coords.height/2),x:parseFloat(ylab.attr('x'))+(lines.length-1)*parseInt(ylabel_textsize)})
		ylab.transform('r270')
		ylab_coords=ylab.getBBox()
	...
	// move the ylabel to the center of the y-axis
