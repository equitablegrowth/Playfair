	...
	Playfair.prototype.linechart = function(options) {
		...
		for (var i=0;i<group.length;i++) {
			...
			for (var k=0;k<group_array.length;k++){
				...
				if ('size' in options){
					minsize=this.dataset['size'][0]
					maxsize=this.dataset['size'][0]

					for(var l=0;l<this.dataset['size'].length;l++){
						if(this.dataset['size'][l]<minsize){
							minsize=this.dataset['size'][l]
						}
						if(this.dataset['size'][l]>maxsize){
							maxsize=this.dataset['size'][l]
						}
					}
				...
				if (options['points']==true) {
					snapobj.circle(x_data_value,y_data_value,pointsize).attr({fill:this.point_fill,stroke:this.qualitative_color[i],'stroke-width':this.point_strokewidth,'data_type':'point','data_label':group_array[k][4],'group':group[i],'class':'dataelement'})
				}
