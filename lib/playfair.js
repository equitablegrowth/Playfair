		...
		for (var i=0;i<group.length;i++) {
			...
			for (var k=0;k<group_array.length;k++){
				...
				if (options['labels']==true) {
					var label=snapobj.text(xa_data_value,y_data_value-pointsize-3,group_array[k][4]).attr({'font-family':this.dataface,'font-size':this.datasize,'font-weight':this.dataweight,'dominant-baseline':'text-before-edge','text-anchor':'middle'})
		...
		if (this.logo!=0){
			...
			logo.node.addEventListener('load',function(){
				...
				if(graphobj.note.length>0){
					...
					note.selectAll("tspan:not(:first-child)").attr({x:source.attr('x'),dy:parseInt(graphobj.notesize)})
					note_coords=note.getBBox().y2
				} else {note_coords=source_coords}
				

// selected_text.selectAll("tspan:not(:first-child)").attr({x:selected_text.attr('x'),dy:selected_text.attr('font-size')})
