...
window.playfair = (function () {
	...
	//			xvar:var1,yvar:var2,connect:var3,grouping:{color:var4}
	//			],
	// 'points': [
	...
	Playfair.prototype.data = function(data,geom_dict) {
		...
		for(var key in geom_dict){
			if(key=='area' | key=='stackedbar'){
				...
				xmaxes.push(Math.max(...geom_dict[key]['xvar']))
				xmins.push(Math.min(...geom_dict[key]['xvar']))
			}
		}
...
Math.max(...all_x)]
	...
	// general method for making charts that are not of a special type - add a geom for each feature.
	// options:
	// min_x - a limit that is not a tick (or at least, doesn't have to be a tick)
	// max_x - upper limit for x
	// min_y
	// max_y
	Playfair.prototype.chart=function(options) {
		...
		// get the min and max of the x axis and the y axis by iterating through all possible variables
		// and getting the max for each. xvar is structured like: {xvar:line:[var1],points:[var2]}
		// first build a list of all x_values and the types of each value.

		// this first if is to check for a custom (user-defined) axis.
		if(typeof this.xarray=='undefined'){
			var all_x=[]
			var x_types=[]
			for(var type in graph_obj.xvar){
				for(var i=0;i<graph_obj.xvar[type].length;i++){
					var temp=graph_obj.dataset[graph_obj[xvar][type][i]]
					for(var j=0;j<temp.length;j++){
						all_x.push(temp[j])
						x_types.push(typeof(temp[j]))
					}
				}
			}

			// if there is more than one type of data in x_types, alert the user - can't do anything with this.
			if(new Set(x_types).size>1){
				alert('Exiting. X variables provided are of different types. Types detected are: '+(new Set(x_types)))
				return
			// Otherwise, create axes appropriate to the data type
			} else if(typeof(x_types[0])=='string'){
				var xaxis=Array.from(new Set(all_x))
				graph_obj.xarray=xaxis
			} else if(typeof(x_types[0])=='number'){
				var xaxis=create_numerical_axis([Math.min(...all_x),Math.max(...all_x)],min_x,max_x)
				graph_obj.xarray=xaxis
			} else if(Object.prototype.toString.call(x_types[0])==='[object Date]'){
				var xaxis=create_date_axis([new Date(moment(Math.min(...all_x),["MM-DD-YYYY","MM/DD/YYYY","YYYY-MM-DD","MM-DD-YY","MM/DD/YY","MMMM YYYY","MMMM DD YYYY","MMMM DD, YYYY","MMMM, YYYY","YYYYqQ"],false)),new Date(moment(Math.max(...all_x),["MM-DD-YYYY","MM/DD/YYYY","YYYY-MM-DD","MM-DD-YY","MM/DD/YY","MMMM YYYY","MMMM DD YYYY","MMMM DD, YYYY","MMMM, YYYY","YYYYqQ"],false))],new Date(moment(min_x,["MM-DD-YYYY","MM/DD/YYYY","YYYY-MM-DD","MM-DD-YY","MM/DD/YY","MMMM YYYY","MMMM DD YYYY","MMMM DD, YYYY","MMMM, YYYY","YYYYqQ"],false)),new Date(moment(max_x,["MM-DD-YYYY","MM/DD/YYYY","YYYY-MM-DD","MM-DD-YY","MM/DD/YY","MMMM YYYY","MMMM DD YYYY","MMMM DD, YYYY","MMMM, YYYY","YYYYqQ"],false)))
				graph_obj.xarray=xaxis
			}
		} else {
	...
	// YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE
	// handle stacked stuff
	// YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE YOU ARE HERE
