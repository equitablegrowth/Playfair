...
window.playfair = (function () {
			...
			if(this.group!=='none'){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					for (var j=0;j<temp_array2.length;j++){
						...
						var bar_rect=snapobj.rect(axes[0]+margin*(i+1)+margin*i+bar_width*i*group.length+bar_width*j,zero-data_value,bar_width,data_value).attr({'data_type':'bar','data_label':temp_array2[j][1],'group':temp_array2[2],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[j]})
					}
				}
			}
		}
	...
	Playfair.prototype.style = function() {
		...
		this.datasize='12px'
		this.dataweight=600
		this.dataface='Lato, Arial, sans-serif'

		// annotations
		this.annotatesize='12px'
		this.annotateweight=600
		this.annotateface='Lato, Arial, sans-serif'
		...
		// x label
		this.xlabel_textsize='14px'
		this.xlabel_textweight=500
		this.xlabel_textface='Lato, Arial, sans-serif'

		// y label
		this.ylabel_textsize='14px'
		this.ylabel_textweight=500
		this.ylabel_textface='Lato, Arial, sans-serif'

		// legend
		this.legend_location='top'
		this.legend_maxwidth=.1
		this.legend_textsize='12px'
		this.legend_textweight=500
		this.legend_textface='Lato, Arial, sans-serif'
		this.legend_toppad=8
		this.legend_bottompad=0
		this.legend_rightpad=0
		this.legend_leftpad=0
		this.legend_elementsize=15
		this.legend_elementpad=5
		...
		this.qualitative_color=['#67c2a5','#fc8d62','#8da0cb','#e78ac3','#a6d854','#ffd92f','#e5c494','#b3b3b3']
	...
	if (playobj.group!=='none'){
		...
		if (playobj.legend_location=='top'){
			...
			for (var i=0;i<group.length;i++){
				...
				console.log(current_linewidth,coords.width,avail_width)
				if (current_linewidth+coords.width+playobj.legend_elementsize+playobj.legend_elementpad<avail_width){
					var temprect=snapobj.rect(horizon_key_start_x+current_linewidth,horizon_key_start_y+playobj.legend_elementpad*line+playobj.legend_toppad+line*lineheight,playobj.legend_elementsize,playobj.legend_elementsize).attr({fill:playobj.qualitative_color[i]})
