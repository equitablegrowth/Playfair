...
function draw_axis(playobj,xvar,yvar) {
	...
	for(var i=0;i<yvar.length;i++){
		var temp=napobj.text(xstart,0,yvar[i]).attr({ident='yaxis','font-size':playobj.ytick_textsize,'font-weight':playobj.ylabel_textweight,'font-face':playobj.ylabel_textface,'dominant-baseline':'text-before-edge','text-anchor':'start'})
		coords=temp.getBBox()
		if (coords.x2>yoffset){yoffset=coords.x2}
	}

	if(yoffset>.1*playobj.width){yoffset=.1*playobj.width}

	// now the full y-offset for the x-axis is known, can start drawing the x-axis.
	if (typeof(playobj.xlabel)!=undefined){
		var xlab=snapobj.text((yoffset+playobj.y+playobj.x-playobj.right_margin)/2,,playobj.ylabel).attr({ident='xaxis','font-size':playobj.xlabel_textsize,'font-weight':playobj.xlabel_textweight,'font-face':playobj.xlabel_textface,'dominant-baseline':'text-before-edge','text-anchor':'middle'})
	}
}
