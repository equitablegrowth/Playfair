...
function draw_axis(playobj,xvar,yvar) {
	...
	if(total_xoffset>.15*playobj.width){total_xoffset=.1*playobj.width}
	...
	x_step=domain/(xvar.length-1)

	// x ticks and lines:
	total_yoffset=playobj.y+playobj.width
	for(var i=0;i<xvar.length;i++){
		var temp=snapobj.text(xstart_xcoord+x_step*i,playobj.y+playobj.height-playobj.footer_bottompad-playobj.logo_height-playobj.footer_toppad-playobj.bottom_margin-xlab_height-playobj.xtick_to_xlabel,String(xvar[i])).attr({ident:'xaxis','font-size':playobj.xtick_textsize,'font-weight':playobj.xlabel_textweight,'font-face':playobj.xlabel_textface,'dominant-baseline':'text-before-edge','text-anchor':'middle'})
		coords=temp.getBBox()
	...
	y_step=range/(yvar.length-1)

	// Now go back to the y ticks and redraw with appropriate coords
	for(var i=0;i<yvar.length;i++){
		var temp=snapobj.text(playobj.x+total_xoffset-playobj.ytick_to_yaxis,ystart_ycoord-y_step*i,String(yvar[i])).attr({ident:'yaxis','font-size':playobj.ytick_textsize,'font-weight':playobj.ylabel_textweight,'font-face':playobj.ylabel_textface,'dominant-baseline':'text-before-edge','text-anchor':'end'})
		coords=temp.getBBox()
