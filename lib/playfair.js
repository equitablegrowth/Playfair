...
window.playfair = (function () {
	...
	Playfair.prototype.linechart = function(options) {
		...
		for (var i=0;i<group.length;i++) {
			...
			for (var k=0;k<group_array.length;k++){
				...
				if (options['labels']==true) {
					var label=snapobj.text(x_data_value,y_data_value-pointsize-3,group_array[k][4]).attr({'font-family':this.dataface,'font-size':this.datasize,'font-weight':this.dataweight,'dominant-baseline':'text-before-edge','text-anchor':'middle',fill:this.datatextfill})
			...
			if ('connect' in options){
				...
				snapobj.path(path_string).attr({'data_label':group[i],class:'dataelement',stroke:color,'stroke-width':2,fill:'none','group':group[i],'fill-opacity':0,'stroke-opacity':this.linechart_strokeopacity})
			}
		}
		...
		for(var i=0;i<keyobjs.length;i++){
			...
			if ('connect' in options){
				if(i<this.qualitative_color.length){color=this.qualitative_color[i]}else{color=this.qualitative_color[this.qualitative_color.length-1]}
				var line=snapobj.line(coords.x,coords.cy,coords.x2,coords.cy).attr({group:group[i],stroke:color,'stroke-width':2,ident:'key',ident2:'keybox','shape-rendering':'crispEdges'})
				// try{floatgroup.append(line)}catch(err){console.log(err)}
			}
			if (options['points']==true){
				if (pointsize>coords.x2-coords.cx){pointsize=coords.x2-coords.cx-4}
				var center=snapobj.circle(coords.cx,coords.cy,pointsize).attr({group:group[i],fill:this.qualitative_color[i],stroke:this.qualitative_color[i],'stroke-width':this.point_strokewidth,ident:'key',ident2:'keybox','fill-opacity':this.point_fillopacity})
		...
		for(var i=0;i<keyobjs.length;i++){
			...
			if ('connect' in options){
				if(i<this.qualitative_color.length){color=this.qualitative_color[i]}else{color=this.qualitative_color[this.qualitative_color.length-1]}
				var line=snapobj.line(coords.x,coords.cy,coords.x2,coords.cy).attr({group:group[i],stroke:color,'stroke-width':2,ident:'key',ident2:'keytop','shape-rendering':'crispEdges'})
				// try{floatgroup.append(line)}catch(err){console.log(err)}
			}
			if (options['points']==true){
				if (pointsize>coords.x2-coords.cx){pointsize=coords.x2-coords.cx-4}
				var center=snapobj.circle(coords.cx,coords.cy,pointsize).attr({group:group[i],fill:this.qualitative_color[i],stroke:this.qualitative_color[i],'stroke-width':this.point_strokewidth,ident:'key',ident2:'keytop','fill-opacity':this.point_fillopacity})
				// try{floatgroup.append(center)}catch(err){console.log(err)}
			}
		}
	}
			...
			if(stacked==true && this.group!=='none'){
				...
				for(var i=0;i<this.dataset[this.xvar].length;i++){
					...
					for(var k=0;k<temp_array.length;k++){
						total=total+temp_array[k][1]
					}

					if (total<min){min=total}
					if (total>max){max=total}
					console.log(temp_array,total)
				}
		...
		if(orientation=='vertical'){
			...
			if(this.group=='none'){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					if(data_value<0){
						var bar_rect=snapobj.rect(axes[0]+margin*(i+1)+margin*i+bar_width*i,zero,bar_width,Math.abs(data_value)).attr({orient:'vertical','data_type':'bar','data_label':this.dataset[this.yvar][i],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[0]})
					}
					else{
						var bar_rect=snapobj.rect(axes[0]+margin*(i+1)+margin*i+bar_width*i,zero-data_value,bar_width,data_value).attr({orient:'vertical','data_type':'bar','data_label':this.dataset[this.yvar][i],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[0]})
					}
				}
			}
			...
			if(this.group!=='none' && stacked==false){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					for (var j=0;j<temp_array2.length;j++){
						...
						var bar_rect=snapobj.rect(axes[0]+margin*(i+1)+margin*i+bar_width*i*group.length+bar_width*j,zero-data_value,bar_width,data_value).attr({orient:'vertical','data_type':'bar','data_label':temp_array2[j][1],'group':temp_array2[j][2],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[j]})
					}
				}
			}
			...
			if(this.group!=='none' && stacked==true){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					for (var j=0;j<temp_array2.length;j++){
						...
						bar_bottom=bar_bottom-data_value
					}
				}
			}
		}
		...
		if(orientation=='horizontal'){
			...
			if(this.group=='none'){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					console.log(axes)
					if(data_value<0){
						var bar_rect=snapobj.rect(zero,axes[3]+margin*(i+1)+margin*i+bar_width*i,data_value,bar_width).attr({orient:'horizontal','data_type':'bar','data_label':this.dataset[this.yvar][i],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[0]})
					}
					else{
						var bar_rect=snapobj.rect(zero,axes[3]+margin*(i+1)+margin*i+bar_width*i,data_value,bar_width).attr({orient:'horizontal','data_type':'bar','data_label':this.dataset[this.yvar][i],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[0]})
					}
			...
			if(this.group!=='none' && stacked==false){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					for (var j=0;j<temp_array2.length;j++){
						...
						var bar_rect=snapobj.rect(zero,axes[3]+margin*(i+1)+margin*i+bar_width*i*group.length+bar_width*j,data_value,bar_width).attr({orient:'horizontal','data_type':'bar','data_label':temp_array2[j][1],'group':temp_array2[j][2],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[j]})
					}
				}
			}
		...
		if (this.logo!=0){
			...
			logo.node.addEventListener('load',function(){
				...
				if(graphobj.source.length>0){
					...
					lines=multitext(source,{'font-family':graphobj.sourceface,'font-size':graphobj.sourcesize,'font-weight':graphobj.sourceweight,'dominant-baseline':'text-before-edge',fill:this.sourcetextfill},graphobj.width-graphobj.footer_leftpad-graphobj.footer_rightpad-logo_coords.width)
				...
				if(graphobj.note.length>0){
					...
					lines=multitext(note,{fill:this.notetextfill,'font-family':graphobj.noteface,'font-size':graphobj.notesize,'font-weight':graphobj.noteweight,'dominant-baseline':'text-before-edge'},graphobj.width-graphobj.footer_leftpad-graphobj.footer_rightpad-logo_coords.width)
					var note=snapobj.text(graphobj.x+graphobj.footer_leftpad,source_coords,lines).attr({fill:this.notetextfill,ident:'foot','font-family':graphobj.noteface,'font-size':graphobj.notesize,'font-weight':graphobj.noteweight,'dominant-baseline':'text-before-edge'})
			...
			if(graphobj.source.length>0){
				...
				var source=snapobj.text(graphobj.x+graphobj.footer_leftpad,graphobj.y+graphobj.height-graphobj.footer_bottompad,lines).attr({ident:'foot','font-family':graphobj.sourceface,fill:this.sourcetextfill,'font-size':graphobj.sourcesize,'font-weight':graphobj.sourceweight,'dominant-baseline':'text-before-edge'})
				source.node.setAttributeNS("http://www.w3.org/XML/1998/namespace", "xml:space", "preserve")
				source.selectAll("tspan:not(:first-child)").attr({x:source.attr('x'),dy:parseInt(graphobj.sourcesize)})
				source_coords=source.getBBox().y2
	...
	Playfair.prototype.header = function(hed,dek) {
		...
		var hed=snapobj.text(this.x+this.header_leftpad,this.y+this.header_toppad,hed).attr({'font-family':this.hedface,'font-size':hedfontsize,'font-weight':this.hedweight,'dominant-baseline':'text-before-edge',fill:this.hedtextfill})
		...
		for(var i=0;i<lines.length;i++){
			var dek=snapobj.text(this.x+this.header_leftpad,this.y+this.header_toppad+hed_coords.y2+i*parseInt(dekfontsize)*1.1,lines[i]).attr({'font-family':this.dekface,'font-size':dekfontsize,'font-weight':this.dekweight,'dominant-baseline':'text-before-edge',ident:'dek',fill:this.dektextfill})
...
function draw_axes(playobj,xvar,yvar,shiftx,shifty) {
	...
	if (playobj.group!=='none' && playobj.legend_location!=='none'){
		...
		longest=0
		for(var i=0;i<group.length;i++){
			var temp=snapobj.text(0,0,group[i]).attr({fill:this.legend_textfill,'font-size':playobj.legend_textsize,'font-weight':playobj.legend_textweight,'font-family':playobj.legend_textface,'dominant-baseline':'text-before-edge','text-anchor':'start'})
		...
		if (playobj.legend_location=='top'){
			...
			for (var i=0;i<group.length;i++){
				...
				if (current_linewidth+coords.width+playobj.legend_elementsize+playobj.legend_elementpad<avail_width){
					var temprect=snapobj.rect(current_linewidth,horizon_key_start_y+playobj.legend_elementpad*line+playobj.legend_toppad+line*lineheight,playobj.legend_elementsize,playobj.legend_elementsize).attr({ident:'key',ident2:'keytop',group:group[i],fill:playobj.qualitative_color[i],ident:'key','shape-rendering':'crispEdges'})
					var temptext=snapobj.text(current_linewidth+playobj.legend_elementpad+playobj.legend_elementsize,horizon_key_start_y+playobj.legend_elementpad*line+playobj.legend_toppad+line*lineheight,text).attr({fill:this.legend_textfill,ident2:'keytop','font-size':playobj.legend_textsize,'font-weight':playobj.legend_textweight,'font-family':playobj.legend_textface,'dominant-baseline':'text-before-edge','text-anchor':'start'})
		...
		if (playobj.legend_location=='float'){
			...
			for (var i=0;i<group.length;i++){
				...
				var temp=snapobj.text(playobj.legend_floatpad+playobj.legend_elementsize+playobj.legend_elementpad,playobj.legend_floatpad+lineheight*i+playobj.legend_elementpad*i,group[i]).attr({fill:this.legend_textfill,ident:'key','font-size':playobj.legend_textsize,'font-weight':playobj.legend_textweight,'font-family':playobj.legend_textface,'dominant-baseline':'text-before-edge','text-anchor':'start'})
				temp.node.setAttributeNS("http://www.w3.org/XML/1998/namespace", "xml:space", "preserve")
			}
			// floatgroup.drag()
	...
	for(var i=0;i<yvar.length;i++){
		...
		var temp=snapobj.text(playobj.left_margin+ylab_width+playobj.ytick_to_ylabel,0,string).attr({fill:this.ytick_textfill,ident:'yaxis','font-size':playobj.ytick_textsize,'font-weight':playobj.ytick_textweight,'font-family':playobj.ytick_textface,'dominant-baseline':'text-before-edge','text-anchor':'start'})
		temp.node.setAttributeNS("http://www.w3.org/XML/1998/namespace", "xml:space", "preserve")
		coords=temp.getBBox()
	...
	for(var i=0;i<xvar.length;i++){
		...
		for(var j=0;j<lines.length;j++){
			var temp=snapobj.text(0,j*parseInt(playobj.xtick_textsize),lines[j]).attr({fill:this.xlabel_textfill,ident:'xaxis','font-size':playobj.xtick_textsize,'font-weight':playobj.xlabel_textweight,'font-family':playobj.xlabel_textface,'dominant-baseline':'text-before-edge','text-anchor':'middle'})
			temp.node.setAttributeNS("http://www.w3.org/XML/1998/namespace", "xml:space", "preserve")
			coords=temp.getBBox()
	...
	for(var i=0;i<xvar.length;i++){
		...
		for(var j=0;j<lines.length;j++){
			...
			var temp=snapobj.text(xstart_xcoord+(x_step/2)*shiftx+x_step*i,playobj.y+playobj.height-playobj.bottom_margin-playobj.footer_height-xlab_height-playobj.xtick_to_xlabel-total_yoffset+playobj.xtick_to_xaxis+j*parseInt(playobj.xtick_textsize),linesj).attr({fill:this.xtick_textfill,ident:'xaxis','font-size':playobj.xtick_textsize,'font-weight':playobj.xtick_textweight,'font-family':playobj.xtick_textface,'dominant-baseline':'text-before-edge','text-anchor':'middle'})
			temp.node.setAttributeNS("http://www.w3.org/XML/1998/namespace", "xml:space", "preserve")
			coords=temp.getBBox()
			temp.attr({y:coords.y-coords.height})
		}
	...
	for(var i=0;i<key_elements.length;i++){
		item=key_elements[i]
		item.attr({x:parseFloat(item.attr('x'))+parseFloat(xstart_xcoord-playobj.ytick_length)})
	}
