...
function create_axis(dataseries,yflag,ticks,decimal,multiple,format,prepend,append) {
	...
	nice_ticks=[.1,.2,.25,.3,.4,.5,.6,.7,.75,.8,.9,1]
	...
	if ((range/(ticks-1))>=1){
	    ...
	    // depends on whether or not 0 is going to be in the array
	    if (zero_flag==1){
	    ...
	    if (zero_flag==0){
	    	for (var i=0;i<candidate_steps.length;i++){
	    		steps=candidate_steps[i]
	    		step_array=[Math.floor(range/steps)]
	    		var stepnum=1
			    while (step_array[step_array.length-1]<maxdata){
			        step_array.push(step_array[0]+steps*stepnum)
			        stepnum++
	...
	for (var i=0;i<candidate_arrays.length;i++){
		...
		if(candidate_arrays[i].length>6){
			penalty=1+.5*(candidate_arrays[i].length-6)
		}

		if (wasted*penalty<best_score){
			best_score=wasted*penalty
			best_array=candidate_arrays[i]
		}

		console.log('array: ',candidate_arrays[i],'score: ',wasted*penalty,'penalty: ',penalty)
	}
