	...
	Playfair.prototype.barchart = function(orientation) {
		...
		if(orientation=='vertical'){
			...
			for (var i=0;i<xaxis.length;i++){
				...
				zero=axes[2]+(-yaxis[0]/data_range)*(axes[2]-axes[3])-1
				data_value=(this.dataset[this.yvar][i]-yaxis[0])/(yaxis[yaxis.length-1]-yaxis[0])*(axes[2]-axes[3])
				console.log(this.dataset[this.yvar][i])
				if(typeof this.group=='undefined'){
					var bar_rect=snapobj.rect(axes[0]+margin*(i+1)+margin*i+bar_width*i,zero-data_value,bar_width,data_value).attr({'shape-rendering':'crispEdges',fill:this.qualitative_color[0]})
				}
