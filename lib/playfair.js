...
window.playfair = (function () {
	...
	Playfair.prototype.linechart = function(options) {
		...
		keyobjs=snapobj.selectAll('rect[ident="key"]')
		for(var i=0;i<keyobjs.length;i++){
			coords=keyobjs[i].getBBox()
			console.log(keyobjs)
			keyobjs[i].remove()
			if ('connect' in options){
				var line=snapobj.line(coords.x,coords.cy-1,coords.x2,coords.cy-1).attr({group:group[i],stroke:this.qualitative_color[i],'stroke-width':2,ident:'key','shape-rendering':'crispEdges'})
				try{floatgroup.append(line)}catch(err){console.log(err)}
			}
			if (options['points']==true){
				if (pointsize>coords.x2-coords.cx){pointsize=coords.x2-coords.cx-4}
				var center=snapobj.circle(coords.cx,coords.cy,pointsize).attr({group:group[i],fill:this.qualitative_color[i],stroke:this.qualitative_color[i],'stroke-width':2,ident:'key','fill-opacity':.2})
