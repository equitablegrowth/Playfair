	...
	Playfair.prototype.linechart = function(orientation) {
		...
		for (var i=0;i<group.length;i++) {
			...
			for (var k=0;k<group_array.length;k++){
				...
				console.log(group_array[k],x_data_value,xaxis,axes)

				snapobj.circle(x_data_value,y_data_value,4).attr({fill:'white'})
				snapobj.circle(x_data_value,y_data_value,3).attr({fill:graph_obj.sequential_color[2]})
			}
		}
	}
