...
window.playfair = (function () {
	...
	Playfair.prototype.linechart = function(options) {
		...
		for (var i=0;i<group.length;i++) {
			...
			for (var k=0;k<group_array.length;k++){
				...
				if (options['points']==true) {
					snapobj.circle(x_data_value,y_data_value,pointsize).attr({fill:this.qualitative_color[i],stroke:this.qualitative_color[i],'stroke-width':this.point_strokewidth,'data_type':'point','data_label':group_array[k][4],'group':group[i],'class':'dataelement','fill-opacity':this.point_fillopacity,colorchange:'both'})
				}

				// label point
				if (options['labels']==true) {
					var label=snapobj.text(x_data_value,y_data_value-pointsize-3,group_array[k][4]).attr({'font-family':this.dataface,'font-size':this.datasize,'font-weight':this.dataweight,'dominant-baseline':'text-before-edge','text-anchor':'middle',fill:this.datatextfill,colorchange:'fill'})
			...
			if ('connect' in options){
				...
				snapobj.path(path_string).attr({'data_label':group[i],class:'dataelement',stroke:color,'stroke-width':2,fill:'none','group':group[i],'fill-opacity':0,'stroke-opacity':this.linechart_strokeopacity,'colorchange':'stroke'})
			}
		}
		...
		for (var i=0;i<options['abline'].length;i++) {
			...
			if (isNumber(ab[0])==true && isNumber(ab[1])==true){
				...
				var trend=snapobj.path(trendstring).attr({stroke:this.trend_fill,'stroke-width':this.trend_width,'colorchange':'stroke'})
				var trendtext=snapobj.text((x1_data_value+x2_data_value)/2,(y1_data_value+y2_data_value)/2,'Trendline').attr({fill:this.trend_textcolor,'font-family':this.trend_textface,'font-size':this.trend_textsize,'font-weight':this.trend_textweight,'dominant-baseline':'text-before-edge','text-anchor':'middle','colorchange':'fill'})
		...
		for(var i=0;i<keyobjs.length;i++){
			...
			if ('connect' in options){
				if(i<this.qualitative_color.length){color=this.qualitative_color[i]}else{color=this.qualitative_color[this.qualitative_color.length-1]}
				var line=snapobj.line(coords.x,coords.cy,coords.x2,coords.cy).attr({group:group[i],stroke:color,'stroke-width':2,ident:'key',ident2:'keybox','shape-rendering':'crispEdges','colorchange':'stroke'})
				// try{floatgroup.append(line)}catch(err){console.log(err)}
			}
			if (options['points']==true){
				if (pointsize>coords.x2-coords.cx){pointsize=coords.x2-coords.cx-4}
				var center=snapobj.circle(coords.cx,coords.cy,pointsize).attr({group:group[i],fill:this.qualitative_color[i],stroke:this.qualitative_color[i],'stroke-width':this.point_strokewidth,ident:'key',ident2:'keybox','fill-opacity':this.point_fillopacity,colorchange:'both'})
		...
		for(var i=0;i<keyobjs.length;i++){
			...
			if ('connect' in options){
				if(i<this.qualitative_color.length){color=this.qualitative_color[i]}else{color=this.qualitative_color[this.qualitative_color.length-1]}
				var line=snapobj.line(coords.x,coords.cy,coords.x2,coords.cy).attr({group:group[i],stroke:color,'stroke-width':2,ident:'key',ident2:'keytop','shape-rendering':'crispEdges','colorchange':'stroke'})
				// try{floatgroup.append(line)}catch(err){console.log(err)}
			}
			if (options['points']==true){
				if (pointsize>coords.x2-coords.cx){pointsize=coords.x2-coords.cx-4}
				var center=snapobj.circle(coords.cx,coords.cy,pointsize).attr({group:group[i],fill:this.qualitative_color[i],stroke:this.qualitative_color[i],'stroke-width':this.point_strokewidth,ident:'key',ident2:'keytop','fill-opacity':this.point_fillopacity,colorchange:'both'})
				// try{floatgroup.append(center)}catch(err){console.log(err)}
			}
		}
	}
		...
		if(orientation=='vertical'){
			...
			if(this.group=='none'){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					if(data_value<0){
						var bar_rect=snapobj.rect(axes[0]+margin*(i+1)+margin*i+bar_width*i,zero,bar_width,Math.abs(data_value)).attr({orient:'vertical','data_type':'bar','data_label':this.dataset[this.yvar][i],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[0],colorchange:'fill'})
					}
					else{
						var bar_rect=snapobj.rect(axes[0]+margin*(i+1)+margin*i+bar_width*i,zero-data_value,bar_width,data_value).attr({orient:'vertical','data_type':'bar','data_label':this.dataset[this.yvar][i],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[0],colorchange:'fill'})
					}
				}
			}
			...
			if(this.group!=='none' && stacked==false){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					for (var j=0;j<temp_array2.length;j++){
						...
						var bar_rect=snapobj.rect(axes[0]+margin*(i+1)+margin*i+bar_width*i*group.length+bar_width*j,zero-data_value,bar_width,data_value).attr({orient:'vertical','data_type':'bar','data_label':temp_array2[j][1],'group':temp_array2[j][2],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[j],colorchange:'fill'})
					}
				}
			}
		...
		if(orientation=='horizontal'){
			...
			if(this.group=='none'){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					console.log(axes)
					if(data_value<0){
						var bar_rect=snapobj.rect(zero,axes[3]+margin*(i+1)+margin*i+bar_width*i,data_value,bar_width).attr({orient:'horizontal','data_type':'bar','data_label':this.dataset[this.yvar][i],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[0],colorchange:'fill'})
					}
					else{
						var bar_rect=snapobj.rect(zero,axes[3]+margin*(i+1)+margin*i+bar_width*i,data_value,bar_width).attr({orient:'horizontal','data_type':'bar','data_label':this.dataset[this.yvar][i],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[0],colorchange:'fill'})
					}
			...
			if(this.group!=='none' && stacked==false){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					for (var j=0;j<temp_array2.length;j++){
						...
						var bar_rect=snapobj.rect(zero,axes[3]+margin*(i+1)+margin*i+bar_width*i*group.length+bar_width*j,data_value,bar_width).attr({orient:'horizontal','data_type':'bar','data_label':temp_array2[j][1],'group':temp_array2[j][2],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[j],colorchange:'fill'})
					}
				}
			}
		...
		if (this.logo!=0){
			...
			logo.node.addEventListener('load',function(){
				...
				if(graphobj.note.length>0){
					...
					var note=snapobj.text(graphobj.x+graphobj.footer_leftpad,source_coords,lines).attr({fill:this.notetextfill,ident:'foot','font-family':graphobj.noteface,'font-size':graphobj.notesize,'font-weight':graphobj.noteweight,'dominant-baseline':'text-before-edge',colorchange:'fill'})
	...
	Playfair.prototype.header = function(hed,dek) {
		...
		var hed=snapobj.text(this.x+this.header_leftpad,this.y+this.header_toppad,hed).attr({'font-family':this.hedface,'font-size':hedfontsize,'font-weight':this.hedweight,'dominant-baseline':'text-before-edge',fill:this.hedtextfill,colorchange:'fill'})
		...
		for(var i=0;i<lines.length;i++){
			var dek=snapobj.text(this.x+this.header_leftpad,this.y+this.header_toppad+hed_coords.y2+i*parseInt(dekfontsize)*1.1,lines[i]).attr({'font-family':this.dekface,'font-size':dekfontsize,'font-weight':this.dekweight,'dominant-baseline':'text-before-edge',ident:'dek',fill:this.dektextfill,colorchange:'fill'})
...
function draw_axes(playobj,xvar,yvar,shiftx,shifty) {
	...
	if (playobj.group!=='none' && playobj.legend_location!=='none'){
		...
		longest=0
		for(var i=0;i<group.length;i++){
			var temp=snapobj.text(0,0,group[i]).attr({fill:this.legend_textfill,'font-size':playobj.legend_textsize,'font-weight':playobj.legend_textweight,'font-family':playobj.legend_textface,'dominant-baseline':'text-before-edge','text-anchor':'start',colorchange:'fill'})
			temp.node.setAttributeNS("http://www.w3.org/XML/1998/namespace", "xml:space", "preserve")
			coords=temp.getBBox()
	...
	if (typeof(playobj.ylabel)!=undefined){
		var ylab=snapobj.text(playobj.x+playobj.left_margin,((playobj.y+playobj.height-playobj.logo_height-playobj.footer_toppad)+(playobj.y+playobj.head_height+playobj.top_margin+legend_height+playobj.legend_toppad+playobj.legend_bottompad))/2,String(playobj.ylabel)).attr({fill:this.ylabel_textfill,ident:'yaxis','font-size':playobj.ylabel_textsize,'font-weight':playobj.ylabel_textweight,'font-family':playobj.ylabel_textface,'dominant-baseline':'central','text-anchor':'middle',colorchange:'fill'})
	...
	for(var i=0;i<xvar.length;i++){
		...
		for(var j=0;j<lines.length;j++){
			var temp=snapobj.text(0,j*parseInt(playobj.xtick_textsize),lines[j]).attr({fill:this.xlabel_textfill,ident:'xaxis','font-size':playobj.xtick_textsize,'font-weight':playobj.xlabel_textweight,'font-family':playobj.xlabel_textface,'dominant-baseline':'text-before-edge','text-anchor':'middle',colorchange:'fill'})
			temp.node.setAttributeNS("http://www.w3.org/XML/1998/namespace", "xml:space", "preserve")
			coords=temp.getBBox()
	...
	for(var i=0;i<xvar.length;i++){
		...
		for(var j=0;j<lines.length;j++){
			...
			var temp=snapobj.text(xstart_xcoord+(x_step/2)*shiftx+x_step*i,playobj.y+playobj.height-playobj.bottom_margin-playobj.footer_height-xlab_height-playobj.xtick_to_xlabel-total_yoffset+playobj.xtick_to_xaxis+j*parseInt(playobj.xtick_textsize),linesj).attr({fill:this.xtick_textfill,ident:'xaxis','font-size':playobj.xtick_textsize,'font-weight':playobj.xtick_textweight,'font-family':playobj.xtick_textface,'dominant-baseline':'text-before-edge','text-anchor':'middle',colorchange:'fill'})
			temp.node.setAttributeNS("http://www.w3.org/XML/1998/namespace", "xml:space", "preserve")
			coords=temp.getBBox()
			temp.attr({y:coords.y-coords.height})
		}
