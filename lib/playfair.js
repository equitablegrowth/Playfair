...
window.playfair = (function () {
			...
			if(this.group!=='none' && stacked==true){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					for (var j=0;j<temp_array2.length;j++){
						...
						var bar_rect=snapobj.rect(axes[0]+margin*(i+1)+margin*i+bar_width*i,bar_bottom-data_value,bar_width,data_value).attr({'data_type':'bar','data_label':temp_array2[j][1],'group':temp_array2[j][2],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[j]})
			...
			if(this.group!=='none' && stacked==true){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					for (var j=0;j<temp_array2.length;j++){
						...
						var bar_rect=snapobj.rect(bar_bottom,axes[3]+margin*(i+1)+margin*i+bar_width*i,data_value,bar_width).attr({'data_type':'bar','data_label':temp_array2[j][1],'group':temp_array2[j][2],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[j]})
						bar_bottom=bar_bottom+data_value
					}
				}
			}
	...
	Playfair.prototype.header = function(hed,dek) {
		...
		console.log('hed',this.hed,this.dek)
