			...
			if(this.group!=='none' && stacked==false){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					for (var j=0;j<temp_array2.length;j++){
						...
						var bar_rect=snapobj.rect(axes[0]+margin*(i+1)+margin*i+bar_width*i*group.length+bar_width*j,zero-data_value,bar_width,data_value).attr({orient:'vertical','data_type':'bar','data_label':temp_array2[j][1],'group':temp_array2[j][2],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[j],colorchange:'fill',context:'data_context_menu'})
					}
				}
			}
			...
			if(this.group!=='none' && stacked==false){
				...
				for (var i=0;i<xaxis.length;i++){
					...
					for (var j=0;j<temp_array2.length;j++){
						...
						var bar_rect=snapobj.rect(zero,axes[3]+margin*(i+1)+margin*i+bar_width*i*group.length+bar_width*j,data_value,bar_width).attr({orient:'horizontal','data_type':'bar','data_label':temp_array2[j][1],'group':temp_array2[j][2],'class':'dataelement','shape-rendering':'crispEdges',fill:this.qualitative_color[j],colorchange:'fill',context:'data_context_menu'})
					}
				}
			}
...
function draw_axes(playobj,xvar,yvar,shiftx,shifty) {
	...
	if (typeof(playobj.ylabel)!=undefined){
		var ylab=snapobj.text(playobj.x+playobj.left_margin,((playobj.y+playobj.height-playobj.logo_height-playobj.footer_toppad)+(playobj.y+playobj.head_height+playobj.top_margin+legend_height+playobj.legend_toppad+playobj.legend_bottompad))/2,String(playobj.ylabel)).attr({fill:this.ylabel_textfill,ident:'yaxis','font-size':playobj.ylabel_textsize,'font-weight':playobj.ylabel_textweight,'font-family':playobj.ylabel_textface,'dominant-baseline':'central','text-anchor':'middle',colorchange:'fill',context:'text_context_menu'})
	...
	for(var i=0;i<xvar.length;i++){
		...
		for(var j=0;j<lines.length;j++){
			var temp=snapobj.text(0,j*parseInt(playobj.xtick_textsize),lines[j]).attr({fill:this.xlabel_textfill,ident:'xaxis','font-size':playobj.xtick_textsize,'font-weight':playobj.xlabel_textweight,'font-family':playobj.xlabel_textface,'dominant-baseline':'text-before-edge','text-anchor':'middle',colorchange:'fill',context:'text_context_menu'})
			temp.node.setAttributeNS("http://www.w3.org/XML/1998/namespace", "xml:space", "preserve")
			coords=temp.getBBox()
