	...
	Playfair.prototype.linechart = function(orientation) {
		...
		for (var i=0;i<group.length;i++) {
			...
			for (var k=0;k<group_array.length;k++){
				...
				x_data_value=(x/(xaxis[xaxis.length-1]-xaxis[0]))*(axes[2]-axes[3])
				console.log(x_data_value)

				y_data_range=yaxis[yaxis.length-1]-yaxis[0]
				y_data_value=(y/(yaxis[yaxis.length-1]-yaxis[0]))*(axes[2]-axes[3])

				snapobj.circle(x_data_value,y_data_value,5).attr({fill:'black'})
			}
		}
	}
