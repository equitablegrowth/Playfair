...
window.playfair = (function () {
	...
	Playfair.prototype.barchart = function(orientation) {
		...
		// category values
		console.log(this.dataset[this.xvar])
		yaxis=create_axis(this.dataset[this.yvar],this.yticks,this.ydecimal,this.ymult,this.yformat,this.ypre,this.yapp)
...
function create_axis(dataseries,yflag,ticks,decimal,multiple,format,prepend,append) {
	...
	if ((range/(ticks-1))>=0){
	    ...
	    for (var i=0;i<nice_ticks.length;i++){
	    	...
	    	if(digits==2){
				candidate_steps[0].push(nice_ticks[i]*Math.pow(10,digits))
				candidate_steps[1].push(nice_ticks[i]*Math.pow(10,digits-1))
				candidate_steps[0].push(nice_ticks[i]*Math.pow(10,digits+1))
	    	}
	    	if(digits<2){
				candidate_steps[2-digits].push(nice_ticks[i]*Math.pow(10,digits))
				candidate_steps[3-digits].push(nice_ticks[i]*Math.pow(10,digits-1))
				candidate_steps[1-digits].push(nice_ticks[i]*Math.pow(10,digits+1))
	    	}
		...
		for (var i=0;i<candidate_steps.length;i++){
		    ...
		    while (step_array[step_array.length-1]<maxdata){
		        step_array.push(step_array[0]+steps*stepnum)
		        stepnum++
		    }
