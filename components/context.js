...
function addtext(target) {
	...
	var text=grapharea.text(target.clientX-svgx,target.clientY-svgy,'Text Annotation').attr({'font-family':annoface,'font-size':annosize,'text-anchor':'start','fill':annofill,'font-weight':annoweight,'dominant-baseline':'hanging':'text-before-edge',cursor:'pointer',colorchange:'fill',context:'text_context_menu',annotation:1})
	...
	if(item.attr('data_type')=='bar'){
		...
		if(item.attr('orient')=='vertical'){
			if(item.attr('data_label')<0){
				var datalabel=grapharea.text(coords.x+coords.width/2,coords.y2,item.attr('data_label')).attr({ident:'foot','font-family':chartobject.data_labels.dataface,'font-size':chartobject.data_labels.datasize,'font-weight':chartobject.data_labels.dataweight,'dominant-baseline':'hanging':'text-before-edge','text-anchor':'middle',colorchange:'fill',context:'text_context_menu',annotation:1})
